...
export const AutoAssignConfirmModal: React.FC = () => {
  const { isOpen, result } = useAppStore(
    (state) => state.ui.autoAssignConfirmModal,
  );
  const {
    setAutoAssignConfirmModalOpen,
    setAllocationConfigOpen,
    applyAutoAssignmentResult,
  ...
  const handleApply = () => {
    ...
    setAutoAssignConfirmModalOpen(false, null);
  };
      ...
      <div className={s.modal}>
        ...
        <div className={s.summaryBox}>
          <div className={s.summaryText}>
            ...
            <span className={s.highlight}>{successCount}</span>
            人の割り当てに成功しました！
          </div>
          {result.success ? (
            <p className={s.successMessage}>
              🎉 全員の希望日程と条件を満たす完璧な割り当てが見つかりました。
            </p>
          ) : (
            <p className={s.warningMessage}>
              ⚠️ 条件が厳しいため、{result.unassigned.length}
              名が未割り当てとなりました。
            </p>
          )}
        </div>
        ...
        {!result.success && (
          <div className={s.unassignedList}>
            <div className={s.unassignedTitle}>
              未割り当ての児童（手動調整または設定の見直しが必要）
            </div>
            <ul
              style={{
                margin: 0,
            ...
            >
              {result.unassigned.map((app) => (
                <li key={app.id}>
                  {app.family_name} {app.first_name}
                </li>
              ))}
            </ul>
          </div>
